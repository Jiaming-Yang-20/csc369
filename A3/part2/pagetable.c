...
static int allocate_frame(pt_entry_t *pte)
{
	...
	{ // Didn't find a free page.
		...
		pt_entry_t *evit_pte = coremap[frame].pte;
		if (evit_pte->frame & PAGE_DIRTY)
		{
			int new_swap_offset = swap_pageout(frame, evit_pte->swap_offset);
			if (new_swap_offset == INVALID_SWAP)
			{
				exit(1);
			}
			evit_pte->swap_offset = new_swap_offset;
			evict_dirty_count += 1;
			evit_pte->frame &= ~PAGE_DIRTY; // evicted not dirty anymore
		}
		else
		{
		...
		evit_pte->frame &= ~PAGE_VALID; //page is evicted so currently not valid
		evit_pte->frame |= PAGE_ONSWAP;
	}
...
char *find_physpage(vaddr_t vaddr, char type)
{
	...
	// IMPLEMENTATION NEEDED
	vaddr_t pdpt_index = PDPT_INDEX(vaddr);
	vaddr_t pd_index = PD_INDEX(vaddr);
	vaddr_t pt_index = PT_INDEX(vaddr);
	...
	uintptr_t pdp = pdpt[pdpt_index].pdp;
	pd_entry_t *pd_start = (pd_entry_t *)(pdp & ~PAGE_VALID); //mask the valid bit to get the addr

	// Use 2nd-level page table and vaddr to get pointer to 3rd-level page table
	if (!(pd_start[pd_index].pde & PAGE_VALID))
	{
		pd_start[pd_index] = init_page_table();
	}
	uintptr_t pde = pd_start[pd_index].pde;
	pt_entry_t *pt_start = (pt_entry_t *)(pde & ~PAGE_VALID);

	// Use 3rd-level page table and vaddr to get pointer to page table entry
	pte = &(pt_start[pt_index]);
	...
	{
		miss_count++;
		unsigned int frame = allocate_frame(pte);
		if (pte->frame & PAGE_ONSWAP)
		{ //page evicted to swap before
